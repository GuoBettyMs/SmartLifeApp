...
/// Image cache type
typedef NS_OPTIONS(NSUInteger, ThingEncryptImageCacheType) {
    /// No value.
    ThingEncryptImageCacheTypeNone   = 0,
    
    /// Get/store image with memory cache.
    ThingEncryptImageCacheTypeMemory = 1 << 0,
    
    /// Get/store image with disk cache.
    ThingEncryptImageCacheTypeDisk   = 1 << 1,
    
    /// Get/store image with both memory cache and disk cache.
    ThingEncryptImageCacheTypeAll    = ThingEncryptImageCacheTypeMemory | ThingEncryptImageCacheTypeDisk,
};

@class ThingEncryptMemoryCache, ThingEncryptDiskCache;
@interface ThingEncryptImageCache : NSObject

/** The underlying memory cache. see `ThingEncryptMemoryCache` for more information.*/
@property (strong, readonly) ThingEncryptMemoryCache *memoryCache;

/** The underlying disk cache. see `ThingEncryptDiskCache` for more information.*/
@property (strong, readonly) ThingEncryptDiskCache *diskCache;

/**
 Whether decode animated image when fetch image from disk cache. Default is YES.
 
 @discussion When fetch image from disk cache, it will use 'ThingInternalEncryptImage' to decode
...
/**
 ...
 @return  The singleton ThingImageCache instance.
 */
...
/**
 Returns the image associated with a given key.
 If the image is not in memory and the `type` contains `ThingEncryptImageCacheTypeDisk`,
 this method may blocks the calling thread until file read finished.
 
 @param key A string identifying the image. If nil, just return nil.
 @return The image associated with key, or nil if no image is associated with key.
 */
- (nullable UIImage *)getImageForKey:(NSString *)key withType:(ThingEncryptImageCacheType)type;
...
/**
 ...
 @discussion If the `type` contain `ThingEncryptImageCacheTypeMemory`, then the `image` will
 be stored in the memory cache; `imageData` will be used instead if `image` is nil.
 If the `type` contain `ThingEncryptImageCacheTypeDisk`, then the `imageData` will
...
/**
 ...
 @return Whether the image is in cache.
 */
- (BOOL)containsImageForKey:(NSString *)key withType:(ThingEncryptImageCacheType)type;
 ...
 */
- (void)removeImageForKey:(NSString *)key withType:(ThingEncryptImageCacheType)type;
...
/**
 ...
 @param completion A completion block which will be called on main thread.
 */
- (void)removeImageForKey:(NSString *)key withType:(ThingEncryptImageCacheType)type completion:(nullable void(^)(void))completion;
