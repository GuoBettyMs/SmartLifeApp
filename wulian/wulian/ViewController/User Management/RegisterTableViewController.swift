...
//                    self.navigationController?.popViewController(animated: true)
                ...
                Alert.showBasicAlert(on: self, with: NSLocalizedString("Registered Anonymous user Successfully", comment: ""), message: NSLocalizedString("匿名用户: \(ThingSmartUser.sharedInstance().nickname)", comment: ""), actions: [action])
                
                print("RegisterTableViewController 注册匿名用户: \(ThingSmartUser.sharedInstance().nickname) 成功")
