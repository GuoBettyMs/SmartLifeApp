...
class MessageDNDDetailViewController : UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        messageSetting.getDNDDeviceListSuccess { result in
            print("MessageDNDDetailViewController 查询所有家庭下所有设备列表成功")
            print(result)
        } failure: { error in
    ...
    func updateDND() {
        ...
        requestModel.loops = loops //每周重复次数
        requestModel.isAllDevIDs = dndModel?.allDevIds ?? false //所有设备是否都支持免打扰设置（DND）当该值为 true 时，devIDs 可以为空
        
        messageSetting.modifyDND(withTimerID: dndModel!.timerId, dndRequestModel: requestModel) {
            print("MessageDNDDetailViewController 修改免打扰消息时段成功")
    ...
    func deleteDND() {
        messageSetting.removeDND(withTimerID: dndModel?.timerId ?? 0) {
            print("MessageDNDDetailViewController 删除免打扰消息时段成功")
            self.navigationController?.popViewController(animated: true)
        } failure: { error in
