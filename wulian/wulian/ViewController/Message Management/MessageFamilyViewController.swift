...
class MessageFamilyViewController : UITableViewController, MessageContentCellDelegate {
    ...
    func loadData() {
        ...
        message.fetchList(with: requestModel) {[weak self] list in
            ...
            print("MessageFamilyViewController 查询消息列表成功")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell?.msgId = messageList[indexPath.row].msgId //消息的 identifiier
    ...
    @IBAction func delete() {
        ...
        requestModel.msgIds = deleteIds //消息 id,NSArray 类型
        requestModel.msgType = .family
        
        message.delete(with: requestModel) { result in
            print("MessageFamilyViewController 删除消息列表成功")
            self.loadData()
        } failure: { error in
